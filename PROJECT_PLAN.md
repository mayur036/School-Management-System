...
| M8  | **Server-Side Pagination, Searching, Filtering, and Sorting** | Move searching, filtering, and sorting to the server. Implement paginated database queries (`LIMIT/OFFSET`, `LIKE` queries, dynamic `ORDER BY` directions inside stored procedures) to support filter criteria (`?page=&limit=&search=&sort=&status=&dept=`). | `server/database/procedures.sql`, models, controllers, client pagination |
